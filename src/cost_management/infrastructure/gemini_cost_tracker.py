...
class EmbeddingPricing:
    """Pricing for Gemini embedding models, in USD per million tokens."""
...
def _lookup_usage_token_count(usage: Any) -> int:
    ...
    if isinstance(usage, Mapping):
        for key in ("input_token_count", "total_token_count", "token_count"):
            if key in usage:
    ...
    return (
        _safe_int(getattr(usage, "input_token_count", 0))
        or _safe_int(getattr(usage, "total_token_count", 0))
        or _safe_int(getattr(usage, "token_count", 0))
    )
...
def _estimate_tokens_from_text(text: Optional[str]) -> int:
    if not text:
        return 0
    approx = int(math.ceil(len(text) / 3.8))
    if approx <= 0 and text.strip():  # pragma: no cover - defensive guard
        return len(text.split()) or 1
    return max(approx, 0)


class GeminiEmbeddingCostTracker:
    """Collect usage statistics and cost estimates for Gemini embeddings."""
    ...
    def configure_from_environment(
        self, env_var: str = "GEMINI_EMBED_PRICE_OVERRIDES"
    ) -> None:
        ...
        for model, data in payload.items():
            ...
            try:
                ...
                _log.warning(
                    "Invalid Gemini embed pricing override for model %s", model
                )
                continue
    ...
    def format_report(self) -> str:
        ...
        if not models:
            return "Gemini embedding usage: no tracked calls."

        lines = ["Gemini embedding usage:"]
...
_default_gemini_pricing: Dict[str, EmbeddingPricing] = {}
_default_price = os.getenv("GEMINI_EMBED_USD_PER_MILLION")
if _default_price:
    try:
        _default_gemini_pricing["*"] = EmbeddingPricing(
            usd_per_million_tokens=float(_default_price)
        )
    except ValueError:
        _log.warning(
            "Invalid GEMINI_EMBED_USD_PER_MILLION value: %s", _default_price
        )


gemini_cost_tracker = GeminiEmbeddingCostTracker(pricing=_default_gemini_pricing)
