...
from src.document_store.infrastructure.bm25_function import (
    BM25_FUNCTION_NAME,
    build_bm25_function,
)
from src.shared.config.config import MilvusSettings
...
class MilvusStore:
    ...
    def document_exists(
        ...
        *,
        ...
        if not workspace:
            return False
        if not doc_name or not document_id:
            return False
        ...
        filter_expr = self._and_filters(
            ...
            self._eq_expr("document_id", document_id),
    ...
    def _build_indexes(self):
        ...
        sparse_metric = self._settings.sparse_metric
        index_params.add_index(
            field_name="sparse_vector",
    ...
    def upsert(self, rows: Iterable[Dict[str, Any]]) -> None:
        ...
        for row in prepared:
            ...
            if row["document_id"] != document_id:
                raise ValueError("All rows in a batch must share the same document_id")
            if row["doc_name"] != doc_name:
                raise ValueError("All rows in a batch must share the same doc_name")

        document_filter = self._and_filters(
            self._eq_expr(self._pk_field, workspace_id),
            self._eq_expr("document_id", document_id),
        )
        existing_same_document = self._client.query(
            collection_name=self._collection,
        ...
        if any(hit.get("doc_name") == doc_name for hit in existing_same_document):
            # Case 2: identical document + name; nothing to do.
            return
        ...
        if existing_same_name and any(hit.get("document_id") != document_id for hit in existing_same_name):
            self._client.delete(
                collection_name=self._collection,
                filter=name_filter,
            )
    ...
    def _prepare_rows(self, rows: Iterable[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for row in rows:
            ...
            metadata.setdefault("document_id", base.get("document_id"))
            ...
            document_id = base.get("document_id") or metadata.get("document_id")
            doc_name = base.get("doc_name") or metadata.get("doc_name")
            if not document_id or not doc_name:
                raise ValueError("Each row must include document_id and doc_name")
