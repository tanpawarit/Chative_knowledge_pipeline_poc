"""Embedding provider implementations shared across contexts."""
...
from src.shared.cost_management.openai_cost_tracker import (
...
class OpenAIEmbedder:
    """Thin wrapper around the OpenAI embeddings API with cost tracking hooks."""
    ...
    def __init__(self, settings: EmbeddingSettings):
        ...
        if override_price:
            self.cost_tracker.configure_pricing(
                self.model,
                OpenAIEmbeddingPricing(usd_per_million_tokens=float(override_price)),
            )
    ...
    def embed_batch(self, texts: List[str]) -> List[np.ndarray]:
        ...
        for i in range(0, len(texts), self.batch_size):
            ...
            for item in batch:
                response = self._client.embeddings.create(model=self.model, input=item)
