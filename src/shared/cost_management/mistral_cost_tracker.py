...
from src.shared.config.config import ExtractionSettings
...
class MistralCostTracker:
    ...
    def configure_from_environment(self, env_var: str = "MISTRAL_PRICE_OVERRIDES") -> None:
        ...
        try:
            ...
            _log.warning(
                "Failed to parse pricing overrides", variable=env_var
            )
            return

        if not isinstance(payload, Mapping):
            _log.warning(
                "Unexpected pricing overrides format",
                expected="mapping",
        ...
        if isinstance(chat_section, Mapping):
            for model, data in chat_section.items():
                ...
                try:
                    ...
                    _log.warning(
                        "Invalid chat pricing override", model=model
                    )
                    continue
        ...
        if isinstance(ocr_section, Mapping):
            for model, data in ocr_section.items():
                ...
                try:
                    ...
                    _log.warning(
                        "Invalid OCR pricing override", model=model
                    )
                    continue
...
if _settings.picture_model:
    _default_chat_pricing[_settings.picture_model] = ChatPricing(
        input_usd_per_million=_settings.picture_input_cost_per_million,
        output_usd_per_million=_settings.picture_output_cost_per_million,
    )

_default_ocr_pricing: Dict[str, OcrPricing] = {}
if _settings.ocr_model:
    _default_ocr_pricing[_settings.ocr_model] = OcrPricing(
        per_page_usd=_settings.ocr_cost_per_page
    )
